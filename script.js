...
function proceedToPlayGame(){
    ...
    proceedToPlay.innerHTML += `<button type="button" class="btn btn-success">Play</button>`;
}
...
function playGame(){
    ...
    if(twoSure.checked){
        if(num1.value == "" || num2.value == ""){
            ...
            content2.innerHTML += `Your input numbers for 2 SURE are: ${arrOfTwoSure} <br>
            These are the generated random numbers: ${randomGeneratedNums}`;

            if(randomGeneratedNums.includes(Number(arrOfTwoSure[0])) && 
            randomGeneratedNums.includes(Number(arrOfTwoSure[1]))){
                content2.innerHTML += `<br><span style='color:green;'>Congratulations! You won</span>`;
            }
            else{
                content2.innerHTML += `<br><span style='color:red;'>You lost! Try again</span>`
            }
        }
    }
    else{
            ...
            (isNaN(num3.value) && !Number.isInteger(num3.value)))
        {
            ...
            content2.innerHTML += `Your input numbers for 3 DIRECT are: ${arrOfThreeDirect} <br>
            These are the generated random numbers: ${randomGeneratedNums}`;
